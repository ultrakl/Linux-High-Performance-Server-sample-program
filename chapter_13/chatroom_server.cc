...
int run_child(int idx, client_data* users, char* share_mem)
{
    ...
    addfd(connfd, child_epollfd);
    int pipefd = users[idx].pipefd[1];
    addfd(pipefd, child_epollfd);
    int ret;
    ...
    while(!stop_server) {
        ...
        for(int i = 0; i < number; i++) {
            ...
            if(listenfd == sockfd) {
                ...
                int connfd = accept(listenfd, (sockaddr*)&client_address, &client_address_length);
                if(connfd == -1) {
                    printf("errno is %d\n", errno);
                    continue;
                }
                        ...
                        switch (signals[i]) {
                            case SIGCHLD: {
                                ...
                                while((pid == waitpid(-1, &stat, WNOHANG)) > 0) {
                                    ...
                                    epoll_ctl(epollfd, EPOLL_CTL_MOD, users[del_user].pipefd[0], 0);
                                    close(users[del_user].pipefd[0]);
                                    users[del_user] = users[user_count--];
                                    sub_process[users[del_user].pid] = del_user;
                                }
                                if(terminate && user_count == 0)stop_server = 0;
                                break;
                            }
                            case SIGTERM: 
                            case SIGINT: {
                ...
                if(ret <= 0)continue;
                    ...
                    for (int j = 0; j < user_count; j++) {
                        if(users[j].connfd != sockfd) {
                            printf("send data to child process\n");
